...
After the fix, re-running the same import volume that triggered the incident kept pool usage below 8/20 throughout the operation.

## Notes
The pool size itself was not the primary issue — the fix to the batch import's connection reuse behavior is the critical change. The pool size increase is a precautionary measure. Monitor pool utilization metrics during future batch imports to confirm the fix holds under varying load conditions. No further action required unless the issue recurs.
